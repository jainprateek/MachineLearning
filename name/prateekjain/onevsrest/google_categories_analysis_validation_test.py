...
samples = []
for idx,row in df.iterrows():
    values = row[['content', 'metatag.keywords']]
    #y.append(values)
    d = dict(zip(features, values))
    samples.append(d)

pprint(samples[0])

#dv = DictVectorizer(sparse=False)
#dv_values = dv.fit_transform(samples)


#tfidfv = TfidfTransformer()
#tfidfv_values = tfidfv.fit_transform(dv_values)
...
#classifier.fit(samples,df.verticals)
#classifier.predict(samples)
scores = cross_validation.cross_val_score(classifier,samples,y_train_mlb, cv=5,scoring='f1_weighted')
print scores
